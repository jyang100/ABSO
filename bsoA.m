function archList= bsoA(func_num,population_size,number_dimension,range_left,range_right,max_iteration, rho)
...
% for idx = 1:population_size
%     [attn_population(idx,1),~] = getNearAttn(population(idx,:), fitness_population(idx,1), population, fitness_population);
% end
...
%             [av, flag] = getNearAttn(individual_temporary, fv, population, fitness_population);
...
if number_dimension == 1 || number_dimension == 2
    ...
    %         end
    if ~isempty(archList)
        archPos = archList(:,1:(size(archList,2)-1));
        archFit = archList(:,size(archList,2));
        if number_dimension == 1
            scatter(archPos, archFit);
            xlabel("x_1")
            ylabel("Fitness value");
        else
